...
- 键值（Key-Value）存储数据库：这一类数据库主要会使用到一个hash表，如Redis、Voldemort、Oracle BDB
...
- KEYS [pattern]			查找出匹配的key【==生成环境禁止使用==】
...
| { **DECR** key }，{ **DECR** key decrement }                 | 自减操作                     |
...
- 使用JedisPool连接池管理连接
  ...
  JedisPool jedisPool = new JedisPool(config, "192.168.1.158", 6379);
...
- slowlog-log-slower-than【慢查询的阙值（微秒），默认10000，建议为1000】
...
如下写法可到达高可用，但不能实现读写分离

```java
public static void main(String[] args) {
    Set<String> sentinels = new HashSet<>();
    sentinels.add("192.168.1.158:26379");
    sentinels.add("192.168.1.158:26380");
    sentinels.add("192.168.1.158:26381");
...
# 十三、<span id="bloomFilter">Redis布隆过滤器</span>
